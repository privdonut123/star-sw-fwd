...
StFstQAMaker::StFstQAMaker( const char* name ) :
  StMaker(name), mEventCounter(0), mDoTreeOutput(true) {
...
Int_t StFstQAMaker::Init()
{
  ...
  if(mDoTreeOutput)
  {
    ...
    //mOutFile = new TFile(mRootFilename.Data(),"RECREATE");
    mOutFile = new TFile("OUTPUT.fstQa.root","RECREATE");
  }

  fstRawHitTree = new TTree("fstRawHits", "fstRawHits_QA");
  fstRawHitTree->Branch("rawHits", &fstRawHit, "channelId/I:geoId:wedge:sensor:phistrip:rstrip:maxTimeBin:rdo:arm:apv:channel:idTruth:seedHitFlag:EventId:charge[3]/F:chargeErr[3]/F");
...
Int_t StFstQAMaker::Finish(){
  ...
  if(mDoTreeOutput)
  {
    ...
    //saving histograms 
    mOutFile->WriteTObject(fstRawHitTree);
    mOutFile->WriteTObject(fstHitTree);

    mOutFile->WriteTObject(numOfRawHits_SensorId);
    mOutFile->WriteTObject(numOfHits_SensorId);
    for(int iTimeBin=0; iTimeBin<kFstNumTimeBins; iTimeBin++)
      mOutFile->WriteTObject(rawHitCharge_TimeBin[iTimeBin]);
    mOutFile->WriteTObject(rawHitChargeErr);
    mOutFile->WriteTObject(hitCharge_SensorId);
    mOutFile->WriteTObject(hitChargeErr_SensorId);
    mOutFile->WriteTObject(rawHitMaxTimeBin_APV);
    mOutFile->WriteTObject(maxTimeBin_SensorId);
    mOutFile->WriteTObject(clusterSize_SensorId);
    mOutFile->WriteTObject(clusterSizeR_SensorId);
    mOutFile->WriteTObject(clusterSizePhi_SensorId);
    for(int iWedge=0; iWedge<kFstNumWedges; iWedge++) {
      for(int iSensor=0; iSensor<kFstNumSensorsPerWedge; iSensor++) {
        mOutFile->WriteTObject(rawHitMap[iWedge*3+iSensor]);
        mOutFile->WriteTObject(hitMap[iWedge*3+iSensor]);
        mOutFile->WriteTObject(numOfRawHits_EventId[iWedge*3+iSensor]);
      }
    }
    for(int iDisk = 0; iDisk < kFstNumDisk; iDisk++)
    {
      mOutFile->WriteTObject(hitMapOfFST[iDisk]);
      mOutFile->WriteTObject(hitMapOfAPV[iDisk]);
      mOutFile->WriteTObject(hitGlobalXY[iDisk]);
      mOutFile->WriteTObject(hitGlobalRPhi[iDisk]);
    }
