...
void qa( TString dataDir = "/Users/brandenburg.89/star/ssw/data/muon_minus/", int tt = -1, TString label = "" ){
    ...
    for (size_t i = 0; i < nEntries; ++i) {
        ...
        for (int j = 0; j < fwdTracks->GetEntriesFast(); ++j) {
            ...
            if ( fwdTrack->idTruth() < 1 ) continue;
    ...
    format_eff_plot(hMatchedMcPrimaryEta, hMcPrimaryEta, "Eta_MatchedPrimary_McPrimary", "Efficiency vs #eta; #eta; Efficiency", kRed);
    format_eff_plot(hMatchedMcPrimaryPt, hMcPrimaryPt, "Pt_MatchedPrimary_McPrimary", "Efficiency vs p_{T}; p_{T} (GeV/c); Efficiency", kRed);
    format_eff_plot(hMatchedMcPrimaryPhi, hMcPrimaryPhi, "Phi_MatchedPrimary_McPrimary", "Efficiency vs #phi; #phi (rad); Efficiency", kRed);
