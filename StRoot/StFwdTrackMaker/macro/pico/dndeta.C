...
void dndeta(    TString dataDir = "/Users/brandenburg.89/star/ssw/data/pythia_jet_nofilter_vz0/", 
            int tt = -1, // track type
            TString label = "pythia",
    ...
    TFile *fOutput = new TFile( TString::Format( "dndeta%d_%s.root", testTrackType, label.Data() ) , "RECREATE");
    fOutput->cd();
    ...
    TH1 * hFwdMultReco = new TH1F("hFwdMultReco", "Reco Mult", nMultBins, multMin, multMax);
    // for MC files, ned to split logic
    TH1 * hFwdMultRecoMC = new TH1F("hFwdMultRecoMC", "Reco Mult", nMultBins, multMin, multMax);
    ...
    for (size_t i = 0; i < nEntries; ++i) {
        ...
        size_t fwdMultRecoMC = 0; // Reconstructed forward multiplicity counter
        ...
        for (int j = 0; j < fwdTracks->GetEntriesFast(); ++j) {
            ...
            if ( fwdTrack->chi2() > 0.0001 && fabs(fwdTrack->numberOfFitPoints()) > 1 ){
                fwdMultReco++;
            }
        ...
        if ( fwdMultReco > 0 ){
            hFwdMultReco->Fill(fwdMultReco);
        }
        hFwdMultRecoMC->Fill(fwdMultRecoMC);
    } // end of event loop
