...
void fwd_tracking(      int n = 500,
    ...
    {
        if ( useFCS )
            _chain = Form("fzin %s sdt20211016 fttFastSim fstFastSim fcsSim fcsWFF fcsCluster fwdTrack MakeEvent StEvent FieldOff agml usexgeom bigbig evout cmudst tree", _geom.Data());
        else 
            _chain = Form("fzin %s sdt20211016 fttFastSim fstFastSim fcsSim fwdTrack MakeEvent StEvent FieldOff agml usexgeom bigbig evout cmudst tree", _geom.Data());
    }
    else
    {
        if ( useFCS )
            _chain = Form("fzin %s sdt20211016 fttFastSim fstFastSim fcsSim fcsWFF fcsCluster fwdTrack MakeEvent StEvent ReverseField agml usexgeom bigbig evout cmudst tree", _geom.Data());
        else 
            _chain = Form("fzin %s sdt20211016 fttFastSim fstFastSim fcsSim fwdTrack MakeEvent StEvent ReverseField agml usexgeom bigbig evout cmudst tree", _geom.Data());
    }
...
//goto chain_loop;
    ...
    // Configure FST FastSim
        ...
        fstFastSim->SetMisalignment(misSensor,deltau,deltav,deltag);
        fstFastSim->SetDebug();
